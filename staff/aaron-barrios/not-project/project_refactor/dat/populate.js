...
db.connect(process.env.MONGO_URL_TEST)
    ...
    .then(csv => {
        ...
        const creations = lines.map(line => {
            const [name, email, username, password, role] = lines.split(',').map(item => item.trim())
    ...
    .then(users => {
        return fs.readFile('./posts.csv', 'utf-8')
            .then(csv => {
                ...
                const creations = lines.map(line => {
                    const [username, text, image, date] = lines.split(',').map(item => item.trim())
                    ...
                    return Post.create({ author, text, image, date, likes })
                })
