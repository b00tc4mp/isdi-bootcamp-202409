...
describe('createPost', () => {
    ...
    it('fails on non-string user-id', () =>
        expect(() => createPost(true, 'https://www.image.com', 'hola world')).to.throw(ValidationError, /^invalid userId$/)
    )

    it('fails on non-24-chars-length user-id', () =>
        expect(() => createPost('0123', 'https://www.image.com', 'hola world')).to.throw(ValidationError, /^invalid userId length$/)
    )

    it('fails on non-string image', () =>
        expect(() => createPost('111222333444555666777888', true, 'hola world')).to.throw(ValidationError, /^invalid image$/)
    )

    it('fails on non-string text', () =>
        expect(() => createPost('111222333444555666777888', 'https://www.image.com', true)).to.throw(ValidationError, /^invalid text$/)
    )
