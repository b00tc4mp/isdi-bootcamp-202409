...
db.connect(process.env.MONGO_URL).then(() => {
    ...
    server.put('/users/:userId/state', authorizationHandler, jsonBodyParser, createFunctionalHandler((req, res) => {
        const { userId, body: { x, y } } = req

        return logic.savePlayerState(userId, x, y).then(() => res.status(204).send())
    }))

    server.get('/users/state', authorizationHandler, createFunctionalHandler((req, res) => {
        const { userId } = req

        return logic.getPlayersState(userId).then(state => res.json(state))
    }))
