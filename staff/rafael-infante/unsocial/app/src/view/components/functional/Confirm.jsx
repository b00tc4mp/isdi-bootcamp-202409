export default function Confirm({ message, level = 'error', onAccepted, onCancelled }) {
  ...
  return (
    <div className="fixed h-full w-full top-0 flex items-center justify-center">
      ...
      >
        <p>{message}</p>
        <button className="border-2 border-black pt-1 bg-gray-400" onClick={handleCancelClick}>
          Cancel
        </button>
        <button className="border-2 border-black pt-1" onClick={handleAcceptClick}>
          Accept
        </button>
      </div>
    </div>
