...
describe('searchProducts', () => {
  ...
  it('succeeds on finding by category', () => {
    ...
    const product1 = new Product({ name: 'NameProduct1', category: 'Face', image: 'https://www.image.com', description: 'Description of product 1' })

    return Promise.all([user.save(), product1.save()])
      .then(([user, product1]) =>
        searchProducts(user.id, 'Face', 'product')
          .then(product => {
  ...
  it('fails on non-existing user', () =>
    expect(
      searchProducts('012345678901234567890123', 'Face')
