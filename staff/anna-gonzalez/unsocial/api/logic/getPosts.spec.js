...
describe('getPosts', () => {
    ...
    it('succeeds for existing user', async () => {
        ...
        const post = await new Post({ author: user.id, image: 'https://www.image.com', text: 'post text', date: new Date(2024, 10, 18) })
        const post2 = await new Post({ author: user.id, image: 'https://www.image.com/2', text: 'post2 text', date: new Date(2024, 10, 19) })
