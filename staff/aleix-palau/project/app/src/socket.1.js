...
// Helper function to emit typing start
export const emitTyping = (matchId) => {
    const currentSocket = getSocket()
    if (currentSocket && currentSocket.connected && matchId) {
        currentSocket.emit('typing', { matchId })
    }
}

// Helper function to emit typing stop
export const emitStopTyping = (matchId) => {
    const currentSocket = getSocket()
    if (currentSocket && currentSocket.connected && matchId) {
        currentSocket.emit('stopTyping', { matchId })
    }
}

// Helper function to emit that messages have been read
export const emitMessagesRead = (matchId, messageIds) => {
    const currentSocket = getSocket()
    if (currentSocket && currentSocket.connected && matchId && messageIds && messageIds.length > 0) {
        console.log(`Emitting messagesRead for match ${matchId}`, messageIds)
        currentSocket.emit('messagesRead', { matchId, messageIds })
    } else {
        console.warn(`Could not emit messagesRead for ${matchId}: Socket not connected or missing data.`)
    }
}
