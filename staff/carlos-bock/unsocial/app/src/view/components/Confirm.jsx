export default function Confirm({ message, level = 'error', onAccepted, onCancelled }) {
    ...
    const handleCancelClick = () => onAccepted()

    const handleAcceptClick = () => onCancelled()

    return <div className="fixed  h-full w-full top-0 flex items-center justify-center">
        <div className={`min-w-[20rem] max-w-[40rem] min-h-[10rem] bg-white ${borderColor} border-[1rem] flex-col items-center justify-center p-2 gap-2`}>
            <p>{message}</p>
