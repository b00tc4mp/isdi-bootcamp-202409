...
describe('createPost', () => {
            ...
            .then(user =>
                createPost(user.id, 'https://upload.wikimedia.org/wikipedia/commons/5/5b/Grib_skov.jpg', 'Bonito bosque')
                    ...
                    .then(post => {
                        ...
                        expect(post.image).to.equal('https://upload.wikimedia.org/wikipedia/commons/5/5b/Grib_skov.jpg')
                        expect(post.text).to.equal('Bonito bosque')
    ...
    it('fails on non-existing user', () =>
        expect(
            createPost('012345678901234567890127', 'https://upload.wikimedia.org', 'Bonito bosque')
        ).to.be.rejectedWith(NotFoundError, /user not found$/)
    )
    it('fails on non-string user-id', () =>
        expect(() => createPost(true, 'https://www.image.com', 'Bonito bosque')).to.throw(ValidationError, /^invalid userId$/)
    )

    it('fails on non-24-chars-length user-id', () =>
        expect(() => createPost('0123', 'https://www.image.com', 'Bonito bosque')).to.throw(ValidationError, /^invalid userId length$/)
    )

    it('fails on non-string image', () =>
        expect(() => createPost('012345678901234567890123', true, 'Bonito bosque')).to.throw(ValidationError, /^invalid image$/)
    )
